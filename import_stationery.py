...
def import_stationery():
    if not os.path.exists('stationery_products.json'):
        print("Error: stationery_products.json not found!")
        return

    with open('stationery_products.json', 'r', encoding='utf-8') as f:
        products_data = json.load(f)

    # Ensure Stationery category exists
    category, _ = Category.objects.get_or_create(
        name='Stationery',
        defaults={'description': 'Premium personalized stationery and office gifts.'}
    )

    print(f"Starting import of {len(products_data)} stationery products...")
    ...
    for data in products_data:
        ...
        filename = f"stationery_{url_slug}.jpg"
        try:
    ...
    print("All stationery products processed.")
