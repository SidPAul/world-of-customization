...
DECOR_KEYWORDS = ['lamp', 'clock', 'nameplate', 'cushion', 'pillow', 'frame', 'stand', 'plaque', 'calendar', 'vase', 'decor', 'light', 'moon', 'star', 'sun', 'wall', 'art']

def get_decor_product_urls(limit=20):
    product_urls = set()
    page = 2  # Start from page 2 to get fresh ones
    while len(product_urls) < limit:
        url = f"{COLLECTION_URL}?page={page}"
        print(f"Finding decor products on page {page}...")
        try:
            ...
            for a in links:
                ...
                if href:
                    ...
                    if '/products/' in full_url and full_url not in product_urls:
                        # Simple keyword check in the link text or slug
                        if any(k in clean_href.lower() or k in name_text for k in DECOR_KEYWORDS):
                            product_urls.add(full_url)
                            found_on_page += 1
                            if len(product_urls) >= limit:
                                break
            
            print(f"  Discovered {found_on_page} decor products (Total: {len(product_urls)})")
...
def main():
    urls = get_decor_product_urls(limit=20)
    products = []
    for url in urls:
    ...
    with open('extra_decor_products.json', 'w') as f:
        json.dump(products, f, indent=4)
    print(f"Done. Saved {len(products)} extra decor products.")
