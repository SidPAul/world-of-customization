...
NAMEPLATE_KEYWORDS = ['name plate', 'nameplate', 'house sign', 'door sign', 'acrylic name', 'wooden name', 'office sign']

def get_nameplate_urls(limit=15):
    product_urls = set()
    page = 1
    while len(product_urls) < limit:
        url = f"{COLLECTION_URL}?page={page}"
        print(f"Finding name plates on page {page}...")
        try:
            ...
            for a in links:
                ...
                if href:
                    ...
                    if '/products/' in full_url and full_url not in product_urls:
                        if any(k in clean_href.lower() or k in name_text for k in NAMEPLATE_KEYWORDS):
                            product_urls.add(full_url)
                            found_on_page += 1
                            if len(product_urls) >= limit:
                                break
            
            print(f"  Discovered {found_on_page} name plates (Total: {len(product_urls)})")
            
            if page > 40: # Search very deep
                break
            page += 1
            time.sleep(1)
...
def main():
    urls = get_nameplate_urls(limit=12)
    products = []
    for url in urls:
    ...
    with open('nameplate_products.json', 'w') as f:
        json.dump(products, f, indent=4)
    print(f"Done. Saved {len(products)} name plate products.")
